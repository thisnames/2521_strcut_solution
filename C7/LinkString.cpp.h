...
#include <malloc.h>
typedef struct node
{	char data;					//存放字符（每个结点存放一个字符）
	struct node *next;			//指针域
} LinkString;					//链串结点类型

void Assign(LinkString *&s,char str[])	//串赋值
{	int i=0;
	LinkString *p,*tc;
...
void DestroyStr(LinkString *&s)		//销毁串
...
LinkString *Concat(LinkString *s,LinkString *t) //串连接
...
LinkString *SubStr(LinkString *s,int i,int j) //求子串
{	int k=1;
...
int Index(LinkString *s,LinkString *t) //串匹配
...
int InsStr(LinkString *&s,int i,LinkString *t) //子串插入:直接在s中插入子串
{	LinkString *p=s,*q,*r;		//p指向s的头结点
	int k=1;
	if (i<1) return 0;			//参数i错误返回0
	while (k<i && p!=NULL)		//从头结点开始找第i-1个结点p
	{	k++;
		p=p->next;
	}
	if (p==NULL) return 0;		//参数i错误返回0
	q=t->next;					//q指向t的第一个数据结点
	while (q!=NULL)				//参数正确将t的所有结点复制并插入到p之后
...
int DelStr(LinkString *&s,int i,int j)	//子串删除,直接在s中删除子串
{
...
LinkString *RepStrAll(LinkString *s,LinkString *s1,LinkString *s2) //子串替换
{
	int i;
	i=Index(s,s1);					//求s1在s中的位置i
	while (i>0)
	{
		DelStr(s,i,StrLength(s1));	//删除子串s1
		InsStr(s,i,s2);				//插入子串s2
		i=Index(s,s1);				//继续求s1在s中的位置i
	}
	return s;
}
void DispStr(LinkString *s)		//输出串
{	LinkString *p=s->next;
	while (p!=NULL)
	{	printf("%c",p->data);
		p=p->next;
